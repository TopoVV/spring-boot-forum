...
public class LoginController {
    ...
    public void authenticate(@Valid @RequestBody LoginRequest loginRequest) {
...
//                .body(LoginResult.success(loginRequest.getUsername()));
        } catch (BadCredentialsException e) {
            log.error("Bad credentials", e);
//            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(LoginResult.badCredentials());
        } catch (DisabledException e) {
            log.error("Account {} Disabled", loginRequest.getUsername());
//            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(LoginResult.accountNotEnabled());
        } catch (RuntimeException e) {
            log.error("An unexpected error happened during authentication");
//            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(LoginResult.unknownError());
        }
    }
